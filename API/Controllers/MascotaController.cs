 ...
 public class MascotaController : BaseApiController
{
    ...
    {
        var Con = await  _unitofwork.Mascotas.GetAllAsync();
        return _mapper.Map<List<MascotaDto>>(Con);
    }
    ...
    {
        var pag = await _unitofwork.Mascotas.GetAllAsync(Pparams.PageIndex, Pparams.PageSize, Pparams.Search);
    ...
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<Mascota>> Put(int id, [FromBody]Mascota rol){
        if(rol == null)
            return NotFound();
