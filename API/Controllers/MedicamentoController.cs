 ...
 public class MedicamentoController : BaseApiController
{
    ...
    {
        var Con = await  _unitofwork.Medicamentos.GetAllAsync();
        return _mapper.Map<List<MedicamentoDto>>(Con);
    }
    ...
    {
        var pag = await _unitofwork.Medicamentos.GetAllAsync(Pparams.PageIndex, Pparams.PageSize, Pparams.Search);
    ...
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<Medicamento>> Put(int id, [FromBody]Medicamento rol){
        if(rol == null)
            return NotFound();
...
// consulta espeficia
   ...
   [ProducesResponseType(StatusCodes.Status500InternalServerError)]
    public async Task<IActionResult> getMedicinacarita(int valor)
    {
        try
        {
            var medCara = await _unitofwork.Medicamentos.GetMedicinaCara(valor);
            return Ok(medCara);
        }
        catch (Exception ex)
        {
   ...
   [ProducesResponseType(StatusCodes.Status500InternalServerError)]
    public async Task<IActionResult> Getlab()
    {
        try
        {
            var labmed = await _unitofwork.Medicamentos.GetMediciaLaboratory();
            return Ok(labmed);
        }
        catch (Exception ex)
        {
