 ...
 public class LaboratorioController : BaseApiController
{
    ...
    {
        var Con = await  _unitofwork.Laboratorios.GetAllAsync();
        return _mapper.Map<List<LaboratorioDto>>(Con);
    }
    ...
    {
        var pag = await _unitofwork.Laboratorios.GetAllAsync(Pparams.PageIndex, Pparams.PageSize, Pparams.Search);
    ...
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<Laboratorio>> Put(int id, [FromBody]Laboratorio rol){
        if(rol == null)
            return NotFound();
