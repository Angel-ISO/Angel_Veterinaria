 ...
 public class PropietarioController : BaseApiController
{
    ...
    {
        var Con = await  _unitofwork.Propietarios.GetAllAsync();
        return _mapper.Map<List<PropietarioDto>>(Con);
    }
    ...
    {
        var pag = await _unitofwork.Propietarios.GetAllAsync(Pparams.PageIndex, Pparams.PageSize, Pparams.Search);
    ...
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<Propietario>> Put(int id, [FromBody]Propietario rol){
        if(rol == null)
            return NotFound();
